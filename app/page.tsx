...
function FilterSortDropdown({ activeFilters, onApply }: any) {
  ...
  let label = "Filter & Sort";
  if (activeCount > 0 || activeFilters.sortBy !== "default") {
    label = `Filters (${activeCount}) ${activeFilters.sortBy !== "default" ? '• Sorted' : ''}`;
  }

  return (
    <div ref={selectRef} className="relative text-sm z-20">
      <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center justify-between min-w-[140px] bg-[#1e1f20] border text-gray-200 rounded-lg px-4 py-2.5 hover:bg-[#2a2b2f] transition ${activeCount > 0 || activeFilters.sortBy !== "default" ? 'border-gray-500' : 'border-[#2a2b2f]'}`}>
        ...
        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" className={`ml-3 text-gray-400 shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`}><path d="M6 8l4 4 4-4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
      </button>
            ...
            <div className="flex flex-col gap-3">
              {["Lenovo Thinkpad E480", "Lenovo Thinkpad E490", "HP Elitebook 840G2", "Asus PRO P1440FA"].map(m => (
                <label key={m} className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <span className="text-sm text-gray-300 group-hover:text-white transition">{m}</span>
                </label>
              ))}
            </div>
          </div>
  ...
  return (
    <div ref={profileRef} className="relative z-20">
      <div className="relative">
        <button onClick={() => setIsOpen(!isOpen)} className="w-10 h-10 rounded-full bg-[#2a2b2f] border border-[#3a3b3f] flex items-center justify-center overflow-hidden hover:ring-2 hover:ring-gray-500 transition focus:outline-none shrink-0">
          {isLoggedIn ? (
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
          )}
        </button>
...
export default function Home() {
  ...
  const [hasUnreadVolyMessage, setHasUnreadVolyMessage] = useState(true); // NEW: Notification state for Voly
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-[#131314] text-gray-200 font-sans overflow-hidden relative">
      ...
      <div className="w-full lg:w-[50%] flex flex-col border-b lg:border-b-0 lg:border-r border-[#2a2b2f] px-6 py-6 lg:px-10 lg:py-8 custom-scrollbar overflow-y-auto shrink-0 relative">
        <div className="max-w-2xl mx-auto w-full pb-10">
          <h1 className="text-2xl lg:text-3xl font-bold mb-1 text-white tracking-tight">Company Asset Sale</h1>
          <p className="text-gray-400 mb-6 text-sm">Select a laptop from the grid to view details and place your bid.</p>
          
          <div className="bg-[#1e1f20] p-6 rounded-2xl border border-[#2a2b2f] shadow-2xl">
            
            {/* CAROUSEL HEADER */}
            <div 
              className="relative w-full h-64 lg:h-80 mb-6 rounded-xl overflow-hidden shadow-md group/carousel bg-black cursor-pointer"
              onClick={() => setIsLightboxOpen(true)}
            >
              {/* NEW: Custom Tooltip replacing the native browser title attribute */}
              <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-widest px-4 py-2 rounded-full opacity-0 group-hover/carousel:opacity-100 transition-opacity pointer-events-none z-30 border border-white/20 shadow-xl">
              ...
              <button 
                ...
                className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/60 text-white flex items-center justify-center transition-colors hover:bg-black border border-white/10 shadow-lg backdrop-blur-sm z-20"
              ...
              <button 
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/60 text-white flex items-center justify-center transition-colors hover:bg-black border border-white/10 shadow-lg backdrop-blur-sm z-20"
              ...
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2 z-20">
                {selectedLaptop.images.map((_: any, i: number) => (
                  <div key={i} className={`w-2 h-2 rounded-full shadow-md ${i === imgIdx ? "bg-white" : "bg-white/40"}`} />
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="w-full lg:w-[55%] flex flex-col">
                <h2 className="text-2xl font-bold text-white leading-tight mb-1">{selectedLaptop.modelType}</h2>
                <p className="text-gray-400 font-mono tracking-wider mb-4">S/N: <span className="text-gray-200">{selectedLaptop.serialNumber}</span></p>
                <div className="mb-5"><span className={`text-xs px-3 py-1.5 rounded font-medium border ${getDefectColor(selectedLaptop.defectType)}`}>{selectedLaptop.defectType}</span></div>
                <p className="text-gray-400 text-sm mb-6 leading-relaxed">{selectedLaptop.description}</p>
                <h3 className="text-xs uppercase tracking-widest text-gray-500 font-bold mb-3">Hardware Specs</h3>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="bg-[#131314] p-3 rounded-lg border border-[#2a2b2f]"><span className="block text-gray-500 text-[10px] uppercase mb-0.5">Processor</span><span className="text-gray-200 font-medium">{selectedLaptop.specs.cpu}</span></div>
                  <div className="bg-[#131314] p-3 rounded-lg border border-[#2a2b2f]"><span className="block text-gray-500 text-[10px] uppercase mb-0.5">Memory</span><span className="text-gray-200 font-medium">{selectedLaptop.specs.ram}</span></div>
                  <div className="bg-[#131314] p-3 rounded-lg border border-[#2a2b2f]"><span className="block text-gray-500 text-[10px] uppercase mb-0.5">Storage</span><span className="text-gray-200 font-medium">{selectedLaptop.specs.storage}</span></div>
                  <div className="bg-[#131314] p-3 rounded-lg border border-[#2a2b2f]"><span className="block text-gray-500 text-[10px] uppercase mb-0.5">OS</span><span className="text-gray-200 font-medium">{selectedLaptop.specs.os}</span></div>
                </div>
              </div>

              {/* COLUMN B: 3D Flip Card */}
              <div className="w-full lg:w-[45%] relative [perspective:1000px] min-h-[350px]">
                <div className={`w-full h-full transition-transform duration-700 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
                  ...
                  <div className="absolute inset-0 [backface-visibility:hidden] flex flex-col justify-center bg-[#131314] p-6 rounded-xl border border-[#2a2b2f]">
                    <div className="flex items-center gap-1.5 bg-[#2a2b2f] px-3 py-1.5 rounded-md text-sm text-gray-300 w-fit mb-8 border border-[#3a3b3f]">
                    ...
                    <p className="text-gray-500 text-[11px] uppercase tracking-wider font-bold mb-2">Current Highest Bid</p>
                    <p className="text-4xl font-bold text-white mb-8">₱{selectedLaptop.currentBid.toLocaleString()}</p>
                    
                    {activeUserBid ? (
                      <button 
                        onClick={() => setBidToCancel({ bidId: activeUserBid.id, laptopId: selectedLaptop.id })} 
                        className="w-full py-3.5 rounded-xl bg-red-500/10 text-red-500 font-bold text-base border border-red-500/20 hover:bg-red-500 hover:text-white active:scale-[0.98] transition-all duration-200 mb-3 flex items-center justify-center cursor-pointer whitespace-nowrap px-2"
                      >
                      ...
                      <button 
                        ...
                        className="w-full py-4 rounded-xl bg-gray-100 text-black font-bold text-lg hover:bg-gray-300 active:scale-[0.98] transition-all duration-200 mb-3 cursor-pointer"
                      >
                        Place Bid
                      </button>
                    )}
                    ...
                    <button 
                      ...
                      className="w-full py-3.5 rounded-xl bg-[#2a2b2f] text-gray-300 font-medium text-sm hover:bg-[#3a3b3f] hover:text-blue-400 active:scale-[0.98] transition-all duration-200 flex items-center justify-center gap-2 border border-[#3a3b3f] mb-6 cursor-pointer"
                    >
                      <img src="https://api.dicebear.com/7.x/bottts/svg?seed=Voly&backgroundColor=transparent" className="w-5 h-5 opacity-80" alt="Voly" />
                      Ask Voly about this
                    </button>
                    
                    <button 
                      onClick={() => setIsFlipped(true)} 
                      className="text-xs text-gray-500 hover:text-gray-300 transition-colors mt-auto text-center w-full underline underline-offset-4 cursor-pointer"
                    >
                  ...
                  <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] flex flex-col bg-[#131314] p-4 lg:p-5 rounded-xl border border-[#2a2b2f]">
                    <div className="flex flex-col h-full">
                      <h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2 mt-2 px-1">
                      ...
                      <ul className="text-xs text-gray-400 space-y-2 list-disc list-inside mb-6 px-1">
                      ...
                      <div className="flex flex-col items-center mt-auto mb-6">
                        <h3 className="text-[10px] uppercase tracking-widest text-gray-500 font-bold mb-3">Accepted Payments</h3>
                        <div className="flex items-center justify-center gap-4">
                          <div className="w-10 h-10 rounded-full bg-[#1e1f20] border border-[#2a2b2f] flex items-center justify-center text-emerald-400 shadow-sm" title="Cash"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="20" height="12" x="2" y="6" rx="2"/><circle cx="12" cy="12" r="2"/><path d="M6 12h.01M18 12h.01"/></svg></div>
                          <div className="w-10 h-10 rounded-full bg-[#1e1f20] border border-[#2a2b2f] flex items-center justify-center shadow-sm overflow-hidden p-2" title="GCash"><img src="https://upload.wikimedia.org/wikipedia/commons/5/52/GCash_logo.svg" alt="GCash" className="w-full h-full object-contain" /></div>
                          <div className="w-10 h-10 rounded-full bg-[#1e1f20] border border-[#2a2b2f] flex items-center justify-center text-gray-300 shadow-sm" title="Bank Transfer"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="5" height="5" rx="1"/><rect x="16" y="3" width="5" height="5" rx="1"/><rect x="3" y="16" width="5" height="5" rx="1"/><path d="M21 16h-3a2 2 0 0 0-2 2v3"/><path d="M21 21v.01"/><path d="M12 7v3a2 2 0 0 1-2 2H7"/><path d="M3 12h.01"/><path d="M12 3h.01"/><path d="M12 16v.01"/><path d="M16 12h1"/><path d="M21 12v.01"/><path d="M12 21v-1"/></svg></div>
                        </div>
                      </div>

                      <button onClick={() => setIsFlipped(false)} className="text-xs text-gray-500 hover:text-gray-300 transition-colors mt-auto text-center w-full underline underline-offset-4 cursor-pointer">
      ...
      <div className="w-full lg:w-[50%] flex flex-col overflow-auto custom-scrollbar bg-[#101112] relative z-0">
        ...
        <div className="sticky top-0 z-40 bg-[#101112]/90 backdrop-blur-xl px-6 lg:px-8 py-5 lg:py-6 flex justify-between items-center border-b border-[#2a2b2f]/50 shadow-sm">
          <h3 className="text-lg lg:text-xl font-semibold text-white truncate">
            Available <span className="text-gray-500 text-sm ml-1 font-normal">({processedLaptops.length})</span>
          </h3>
          <div className="flex items-center gap-3 lg:gap-4 shrink-0">
            <FilterSortDropdown activeFilters={activeFilters} onApply={(f: any) => { setActiveFilters(f); setVisibleCount(6); }} />
            <div className="h-8 w-px bg-[#2a2b2f] hidden sm:block"></div>
            <ProfileDropdown 
              user={user}
        ...
        <div className="p-6 lg:p-8 pt-6">
          ...
          {visibleCount < processedLaptops.length && (
            <div className="mt-10 flex justify-center pb-10">
              <button onClick={() => setVisibleCount(prev => prev + 6)} className="px-6 py-2.5 rounded-full border border-gray-600 text-gray-300 font-medium hover:bg-[#1e1f20] hover:text-white active:scale-[0.98] transition-all duration-200 text-sm cursor-pointer">
                Load More Laptops
              </button>
            </div>
          )}
          <div className={compareIds.length > 0 ? "pb-32" : "pb-10"} /> 
        </div>
      </div>
      ...
      {/* ======================================================== */}
      {compareIds.length > 0 && (
        <div className="fixed bottom-6 lg:bottom-10 right-6 lg:right-1/4 translate-x-0 lg:translate-x-1/2 z-[80] bg-[#1e1f20]/95 backdrop-blur-xl border border-blue-500/30 shadow-[0_0_30px_rgba(59,130,246,0.15)] rounded-full pl-6 pr-3 py-2 flex items-center gap-4 animate-in slide-in-from-bottom-10 duration-300">
          <span className="text-sm font-bold text-white">{compareIds.length} <span className="font-normal text-gray-400">selected</span></span>
          <div className="h-5 w-px bg-[#3a3b3f]"></div>
          <button 
            id="compare-voly-btn"
            onClick={() => {
            ...
            className="text-sm font-bold text-blue-400 hover:text-white bg-blue-500/10 hover:bg-blue-500/30 px-4 py-2 rounded-full transition-all cursor-pointer flex items-center gap-2"
          >
            <img src="https://api.dicebear.com/7.x/bottts/svg?seed=Voly&backgroundColor=transparent" className="w-4 h-4 opacity-80" alt="Voly" />
            Compare with Voly
          </button>
          <button onClick={() => setCompareIds([])} className="w-8 h-8 flex items-center justify-center rounded-full bg-[#131314] hover:bg-red-500/20 text-gray-400 hover:text-red-400 transition-colors cursor-pointer border border-[#2a2b2f]">
      ...
      <div className="fixed bottom-6 left-6 lg:left-10 z-[90] flex flex-col items-start gap-3 pointer-events-none">
        
        {/* NEW: Unread Sneak Peek Bubble */}
        ...
        <button 
          ...
          className="pointer-events-auto bg-[#1e1f20]/90 backdrop-blur-md border border-[#2a2b2f] shadow-2xl rounded-full pl-2 pr-5 py-2 flex items-center gap-3 transition-all duration-300 hover:scale-105 hover:bg-[#2a2b2f] group relative"
        >
          <div className="relative">
            <img src="https://api.dicebear.com/7.x/bottts/svg?seed=Voly&backgroundColor=131314" alt="Voly Logo" className="w-10 h-10 rounded-full border border-[#3a3b3f]" />
            {/* NEW: Unread Red Badge on Voly */}
            {hasUnreadVolyMessage && (
              <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white shadow-sm border-2 border-[#1e1f20]">
                1
              </span>
            )}
          </div>
          <span className="text-sm font-bold text-white group-hover:text-blue-400 transition-colors tracking-wide">Ask Voly</span>
        </button>
      </div>

      <div 
        ref={volyContainerRef}
        className={`fixed bottom-24 left-6 lg:left-10 z-[100] w-[340px] md:w-[380px] h-[500px] max-h-[70vh] bg-[#101112]/85 backdrop-blur-3xl border border-[#2a2b2f] rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden transition-all duration-300 origin-bottom-left ${isVolyOpen ? 'scale-100 opacity-100 translate-y-0' : 'scale-50 opacity-0 pointer-events-none translate-y-10'}`}
      >
        ...
        <div ref={volyScrollRef} className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 custom-scrollbar">
          ...
          {volyHistory.map((msg, i) => (
            <div key={i} className={`flex flex-col max-w-[90%] ${msg.role === 'user' ? 'self-end items-end' : 'self-start items-start'}`}>
              <div className={`px-4 py-3 rounded-xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-sm' : 'bg-[#1e1f20] text-gray-200 border border-[#2a2b2f] shadow-lg rounded-bl-sm'}`}>
          ...
          {volyHistory.length === 1 && (
            <div className="flex flex-wrap justify-start gap-2 mt-1">
              <button onClick={() => handleVolyMessage("Find me a laptop good for basic school tasks.")} className="bg-[#1e1f20] hover:bg-[#2a2b2f] text-[11px] border border-[#2a2b2f] px-3 py-1.5 rounded-full transition-colors cursor-pointer text-gray-300 shadow-sm">"Good for school tasks"</button>
              <button onClick={() => handleVolyMessage("Show me laptops that are damaged visually but work fine.")} className="bg-[#1e1f20] hover:bg-[#2a2b2f] text-[11px] border border-[#2a2b2f] px-3 py-1.5 rounded-full transition-colors cursor-pointer text-gray-300 shadow-sm">"Only aesthetic damage"</button>
            </div>
          )}
        ...
        <div className="p-3 border-t border-[#2a2b2f] bg-[#1a1b1e]">
          <div className="relative flex items-center">
            <input 
              ...
              className="w-full bg-[#131314] border border-[#2a2b2f] rounded-lg pl-4 pr-12 py-3 text-sm text-white outline-none focus:border-blue-500/50 transition shadow-inner" 
            />
            <button 
      ...
      {/* LIGHTBOX OVERLAY */}
      {isLightboxOpen && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-sm flex items-center justify-center overflow-hidden" onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onMouseMove={handleMouseMove}>
          ...
          <button onClick={(e) => { e.stopPropagation(); setImgIdx(prev => (prev === 0 ? selectedLaptop.images.length - 1 : prev - 1)); }} className="absolute left-6 top-1/2 -translate-y-1/2 text-white/60 hover:text-white z-50 p-4 bg-white/5 rounded-full hover:bg-white/10 transition cursor-pointer"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M15 18l-6-6 6-6"/></svg></button>
          <button onClick={(e) => { e.stopPropagation(); setImgIdx(prev => (prev === selectedLaptop.images.length - 1 ? 0 : prev + 1)); }} className="absolute right-6 top-1/2 -translate-y-1/2 text-white/60 hover:text-white z-50 p-4 bg-white/5 rounded-full hover:bg-white/10 transition cursor-pointer"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M9 18l6-6-6-6"/></svg></button>
          <div className="w-full h-full flex items-center justify-center p-12" onDoubleClick={handleDoubleClick} onMouseDown={handleMouseDown}>
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 text-white/50 text-sm pointer-events-none font-medium tracking-wide bg-black/40 px-4 py-2 rounded-full">Double-click to zoom in/out. Drag to pan.</div>
        </div>
      )}
      ...
      {authErrorMsg && (
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1e1f20] border border-[#2a2b2f] rounded-2xl w-full max-w-sm p-8 shadow-2xl text-center flex flex-col items-center">
            ...
            <p className="text-gray-400 text-sm mb-8 leading-relaxed">{authErrorMsg}</p>
            <button 
          ...
          <div className="bg-[#1e1f20] border border-[#2a2b2f] rounded-2xl w-full max-w-sm p-8 shadow-2xl text-center flex flex-col items-center">
            ...
            <div className="flex gap-3 w-full">
              <button 
                ...
                className="flex-1 py-3 rounded-xl bg-[#2a2b2f] text-gray-300 font-bold hover:bg-[#3a3b3f] active:scale-[0.98] transition-all duration-200 cursor-pointer"
              >
                Cancel
              </button>
              <button 
                onClick={() => { setIsLoginRequiredOpen(false); openLogin(); }} 
                className="flex-1 py-3 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-500 active:scale-[0.98] transition-all duration-200 cursor-pointer shadow-lg"
              >
                Log In Now
              </button>
            </div>
      ...
      {isLoginModalOpen && (
        <div className="fixed inset-0 z-[110] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1e1f20] border border-[#2a2b2f] rounded-2xl w-full max-w-sm p-8 shadow-2xl relative">
            ...
            <div className="space-y-4">
              ...
              <button 
                onClick={handleAuth}
                className="w-full py-3.5 rounded-xl bg-gray-100 text-black font-bold text-base hover:bg-gray-300 active:scale-[0.98] transition-all duration-200 mt-2 cursor-pointer shadow-lg"
              >
            ...
            <div className="mt-6 flex flex-col gap-3 text-center">
              {!isForgotPass && !isSignUp && (
                <button onClick={() => setIsForgotPass(true)} className="text-sm text-gray-400 hover:text-white transition cursor-pointer">
                  Forgot your password?
                </button>
              )}
              
              {isForgotPass ? (
                <button onClick={() => setIsForgotPass(false)} className="text-sm text-gray-400 hover:text-white transition cursor-pointer underline underline-offset-4">
                  Back to Sign In
                </button>
              ) : (
                <button onClick={() => setIsSignUp(!isSignUp)} className="text-sm text-gray-400 hover:text-white transition cursor-pointer underline underline-offset-4">
      ...
      {isBidModalOpen && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1e1f20] border border-[#2a2b2f] rounded-2xl w-full max-w-md p-6 lg:p-8 shadow-2xl relative">
            ...
            <p className="text-gray-400 text-sm mb-6 pb-4 border-b border-[#2a2b2f]">You are bidding on: <span className="text-gray-200 font-semibold">{selectedLaptop.modelType} ({selectedLaptop.serialNumber})</span></p>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="number" step="100" min={selectedLaptop?.currentBid + 100} placeholder={`Minimum ₱${selectedLaptop?.currentBid + 100}`} value={bidForm.amount} onChange={(e) => setBidForm({...bidForm, amount: e.target.value})} className="w-full bg-[#131314] border border-[#2a2b2f] text-white font-bold rounded-lg px-4 py-3 focus:ring-2 focus:ring-gray-500 outline-none text-lg placeholder-gray-600 transition" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Receive Mode</label>
              ...
              {bidForm.receiveMode === "Delivery" && (
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Select Courier</label>
              ...
              <div className="pt-2 pb-2">
                <label className="flex items-start gap-3 cursor-pointer group mb-3">
                  <input type="checkbox" checked={bidForm.agreeAsIs} onChange={(e) => setBidForm({...bidForm, agreeAsIs: e.target.checked})} className="mt-0.5 w-4 h-4 rounded border-[#2a2b2f] bg-[#131314] text-gray-400 focus:ring-gray-500 focus:ring-offset-[#1e1f20] cursor-pointer" />
                  <span className="text-xs text-gray-400 group-hover:text-gray-300 transition-colors leading-relaxed select-none">I acknowledge that this laptop is sold on an <span className="text-gray-200 font-semibold">as-is basis</span> and I have reviewed the listed defects.</span>
                </label>
                <label className="flex items-start gap-3 cursor-pointer group">
                  <input type="checkbox" checked={bidForm.agreeNoWarranty} onChange={(e) => setBidForm({...bidForm, agreeNoWarranty: e.target.checked})} className="mt-0.5 w-4 h-4 rounded border-[#2a2b2f] bg-[#131314] text-gray-400 focus:ring-gray-500 focus:ring-offset-[#1e1f20] cursor-pointer" />
                  <span className="text-xs text-gray-400 group-hover:text-gray-300 transition-colors leading-relaxed select-none">I understand that there is <span className="text-red-400/90 font-medium">no warranty</span> included with this purchase.</span>
                </label>
              </div>
              ...
              <button 
                ...
                className={`w-full py-4 rounded-xl font-bold text-lg transition-all duration-200 shadow-lg mt-2 ${(!isFormValid || isSubmittingBid) ? "bg-[#2a2b2f] text-gray-500 cursor-not-allowed" : "bg-gray-100 text-black hover:bg-gray-300 active:scale-[0.98] cursor-pointer"}`}
              >
      ...
      {isSuccessModalOpen && (
        <div className="fixed inset-0 z-[110] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1e1f20] border border-[#2a2b2f] rounded-2xl w-full max-w-sm p-8 shadow-2xl text-center flex flex-col items-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">Bid Successful!</h2>
            <p className="text-gray-400 text-sm mb-8 leading-relaxed">
      ...
      {bidToCancel && (
        <div className="fixed inset-0 z-[130] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#1e1f20] border border-[#2a2b2f] rounded-2xl w-full max-w-sm p-6 shadow-2xl text-center flex flex-col items-center">
            <div className="w-14 h-14 rounded-full bg-red-500/10 text-red-500 flex items-center justify-center mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"></path><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"></path><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"></path></svg>
            </div>
            <h2 className="text-xl font-bold text-white mb-2">Cancel your bid?</h2>
            <p className="text-gray-400 text-sm mb-8 leading-relaxed">
            ...
            <div className="flex gap-3 w-full">
              <button 
                ...
                className="flex-1 py-3 rounded-xl bg-[#2a2b2f] text-gray-300 font-bold hover:bg-[#3a3b3f] transition-all duration-200 cursor-pointer disabled:opacity-50"
              >
                Keep Bid
              </button>
              <button 
                ...
                disabled={isCanceling}
                className="flex-1 py-3 rounded-xl bg-red-600 text-white font-bold hover:bg-red-500 active:scale-[0.98] transition-all duration-200 cursor-pointer disabled:opacity-50"
              >
      ...
      <>
        ...
        <div className={`fixed inset-y-0 right-0 z-[120] w-full max-w-md bg-[#131314] border-l border-[#2a2b2f] shadow-2xl flex flex-col transform transition-transform duration-300 ease-out ${isMyBidsOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-6 custom-scrollbar flex flex-col gap-5">
            {myBids.length === 0 ? (
               <div className="flex flex-col items-center justify-center h-full text-center opacity-50">
                 ...
                 <p className="text-sm font-medium">You haven't placed any bids yet.</p>
               </div>
            ) : (
              myBids.map((bid) => (
                <div 
                  key={bid.id}
                  className={`bg-[#1e1f20] border rounded-xl p-5 flex flex-col gap-3 relative overflow-hidden cursor-pointer group transition-colors hover:bg-[#232426] ${bid.status === 'winning' ? 'border-emerald-500/30' : 'border-red-500/30'}`}
                ...
                >
                  ...
                  <div className={`absolute inset-0 z-20 bg-black/60 backdrop-blur-[2px] flex items-center justify-center transition-opacity duration-200 ${activeCartItem === bid.id ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                    <button 
                      onClick={(e) => { e.stopPropagation(); setSelectedLaptop(bid.laptop); setIsMyBidsOpen(false); setActiveCartItem(null); }}
                  ...
                  <div className="flex gap-4 relative z-10 w-full">
                    <img src={bid.laptop.images[0]} className={`w-20 h-20 object-cover rounded-lg shrink-0 ${bid.status === 'outbid' ? 'opacity-60 grayscale-[30%]' : ''}`} alt="Laptop" />
                    <div className="flex-1 flex justify-between gap-2">
                      <div className="flex flex-col items-start justify-between py-0.5">
                        <div>
                          <h4 className="text-sm font-bold text-white leading-tight mb-0.5">{bid.laptop.modelType}</h4>
                          <p className="text-[10px] text-gray-500 font-mono mb-1.5">{bid.laptop.serialNumber}</p>
                        </div>
                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-medium border mb-1.5 ${getDefectColor(bid.laptop.defectType)}`}>
                      ...
                      <div className="flex flex-col items-end justify-between py-0.5 text-right shrink-0">
                        <span className={`text-[9px] font-bold px-2 py-1 rounded border ${bid.status === 'winning' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                          {bid.status === 'winning' ? 'Winning' : `Outbid (₱${bid.currentHighest.toLocaleString()})`}
                        </span>
                        <div className="mt-auto pt-2">
                          <p className="text-[9px] text-gray-500 uppercase font-semibold mb-0.5">Your Bid</p>
                          <p className={`text-base font-bold leading-none ${bid.status === 'winning' ? 'text-emerald-400' : 'text-gray-400 line-through decoration-red-500/70'}`}>
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-[#2a2b2f] relative z-10">
                    <button 
                      ...
                      className="text-[10px] text-gray-500 hover:text-red-400 uppercase font-bold tracking-wider transition-colors cursor-pointer"
                    >
                      Cancel Bid
                    </button>
                    {bid.status === 'outbid' && (
                      <button 
                        ...
                        className="text-[10px] bg-blue-600 hover:bg-blue-500 text-white uppercase font-bold tracking-wider px-3 py-1.5 rounded transition-colors shadow-md cursor-pointer"
                      >
          ...
          <div className="p-6 border-t border-[#2a2b2f] bg-[#1a1b1e]">
            <p className="text-xs text-gray-400 text-center">You will receive an email notification if the status of your bids changes.</p>
          </div>
        </div>
      </>
